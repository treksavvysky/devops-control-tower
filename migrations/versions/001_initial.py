...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column('priority', sa.Enum('low', 'medium', 'high', 'critical', name='event_priority'), 
                 nullable=False, default='medium', index=True),
        sa.Column('tags', sa.JSON, nullable=False, default=dict),
        sa.Column('status', sa.Enum('pending', 'processing', 'completed', 'failed', name='event_status'), 
    ...
    op.create_table(
        ...
        sa.Column('status', sa.Enum('idle', 'running', 'completed', 'failed', 'cancelled', name='workflow_status'), 
                 nullable=False, default='idle', index=True),
        sa.Column('created_at', sa.DateTime(timezone=True), nullable=False, server_default=sa.func.now()),
    ...
    op.create_table(
        ...
        sa.Column('status', sa.Enum('inactive', 'starting', 'running', 'stopping', 'error', name='agent_status'), 
                 nullable=False, default='inactive', index=True),
        sa.Column('last_heartbeat', sa.DateTime(timezone=True), nullable=True),
        sa.Column('health_status', sa.String(20), nullable=False, default='unknown', index=True),
