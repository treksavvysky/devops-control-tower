...
class WorkflowTemplates:
    ...
    def deployment_pipeline() -> Workflow:
        ...
        return (
            WorkflowBuilder("deployment_pipeline", "Automated deployment pipeline")
            .triggered_by(EventTypes.CODE_COMMIT)
            .add_step("run_tests", lambda ctx: {"tests_passed": True})
            .add_step(
                "build_image",
                lambda ctx: {"image": "app:latest"},
                dependencies=["run_tests"],
            )
            .add_step(
                "deploy_staging",
                lambda ctx: {"deployed": True},
                dependencies=["build_image"],
            )
            .add_step(
                "run_e2e_tests",
                lambda ctx: {"e2e_passed": True},
                dependencies=["deploy_staging"],
            )
            .add_step(
                "deploy_production",
                lambda ctx: {"production_deployed": True},
                dependencies=["run_e2e_tests"],
            )
            .build()
        )
    ...
    def security_scan() -> Workflow:
        ...
        return (
            WorkflowBuilder("security_scan", "Automated security scanning workflow")
            ...
            .add_step(
                "generate_report",
                lambda ctx: {"report": "security_report.pdf"},
                dependencies=[
