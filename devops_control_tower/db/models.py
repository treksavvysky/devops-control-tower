...
class TaskModel(Base):
    """SQLAlchemy model for tasks (JCT V1 Task Spec)."""

    __tablename__ = "tasks"

    # Primary fields
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    version = Column(String(10), nullable=False, default="1.0", index=True)
    ...
    # Execution details
    assigned_to = Column(String(100), nullable=True)  # Agent or worker ID
    result = Column(JSON, nullable=True)
    error = Column(Text, nullable=True)
