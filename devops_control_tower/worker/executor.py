...
class StubExecutor(Executor):
    ...
    def execute(self, context: ExecutionContext, store: TraceStore) -> ExecutionResult:
        ...
        store.append_line(
            "trace.log", f"  Target: {context.target_repo}:{context.target_ref}"
        )
...
"""
        ...
        store.append_line(
            "trace.log", f"[{completed_at.isoformat()}] Execution completed"
        )
...
def get_executor(executor_type: str = "stub") -> Executor:
    ...
    if executor_type == "stub":
        ...
        raise ValueError(
            f"Unsupported executor type: {executor_type}. " f"Supported: stub"
        )
